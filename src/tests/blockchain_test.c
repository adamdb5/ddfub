...
void get_block_hash_valid(void **state)
{
  ...
  memcpy(block.author, "192.168.1.1", 11);
  memcpy(block.last_hash, "0", 1);
  memcpy(block.rule.source_addr, "100.100.100.100", 15);
  memcpy(block.rule.dest_addr,   "200.200.200.200", 15);
  block.rule.source_port = 1;
  block.rule.dest_port = 1;
  block.rule.action = DENY;
  memcpy((void*)expected_memory,
	 "\xdd\xe1\xe0\x7d\x0b\x78\xe3\x42\x0a\x29\x8c\x4b\xd3\x64\x1f" \
	 "\xaa\x3c\x1d\x9e\x12\x22\x6a\x87\x2a\xe2\x2b\x5b\x68\xce\x6a\xba\xdc",
